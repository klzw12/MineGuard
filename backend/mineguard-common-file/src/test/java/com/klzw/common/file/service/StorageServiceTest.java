 ...
 * StorageService 单元测试
 * <p>
 * 使用Mockito模拟AliyunOssStorageStrategy，测试存储服务的业务逻辑
 */
...
class StorageServiceTest {
    ...
    @BeforeEach
    void setUp() {
        storageService = new AliyunOssStorageServiceImpl(aliyunOssStorageStrategy);
    }
    ...
    void testUpload_Success() {
        ...
        when(aliyunOssStorageStrategy.upload(any(InputStream.class), anyString(), anyString()))
        ...
        verify(aliyunOssStorageStrategy, times(1)).upload(any(InputStream.class), eq("test.png"), eq("image/png"));
    }
    ...
    void testUploadWithModuleAndFolder_Success() {
        ...
        verify(aliyunOssStorageStrategy, times(1)).upload(any(InputStream.class), contains("avatar/"), eq("image/png"));
    }
    ...
    void testDownload_Success() {
        ...
        when(aliyunOssStorageStrategy.download(anyString())).thenReturn(expectedStream);

        InputStream result = storageService.download("test-bucket/test-file.png");

        assertNotNull(result);
        verify(aliyunOssStorageStrategy, times(1)).download("test-bucket/test-file.png");
    }

    @Test
    @DisplayName("测试删除文件 - 成功")
    void testDelete_Success() {
        when(aliyunOssStorageStrategy.delete(anyString())).thenReturn(true);

        boolean result = storageService.delete("test-bucket/test-file.png");

        assertTrue(result);
        verify(aliyunOssStorageStrategy, times(1)).delete("test-bucket/test-file.png");
    }

    @Test
    @DisplayName("测试删除文件 - 失败")
    void testDelete_Failure() {
        when(aliyunOssStorageStrategy.delete(anyString())).thenReturn(false);

        boolean result = storageService.delete("non-existent-file.png");

        assertFalse(result);
        verify(aliyunOssStorageStrategy, times(1)).delete("non-existent-file.png");
    }

    @Test
    @DisplayName("测试获取文件URL - 成功")
    void testGetUrl_Success() {
        String expectedUrl = "https://oss-cn-beijing.aliyuncs.com/test-bucket/test-file.png?signature=xxx";
        when(aliyunOssStorageStrategy.getUrl(anyString(), anyLong())).thenReturn(expectedUrl);

        String result = storageService.getUrl("test-bucket/test-file.png", 3600);

        assertNotNull(result);
        assertEquals(expectedUrl, result);
        verify(aliyunOssStorageStrategy, times(1)).getUrl("test-bucket/test-file.png", 3600);
    }
    ...
    void testGetFileInfo_Success() {
        ...
        when(aliyunOssStorageStrategy.getFileInfo(anyString())).thenReturn(expectedInfo);
        ...
        verify(aliyunOssStorageStrategy, times(1)).getFileInfo("test-bucket/test-file.png");
    }

    @Test
    @DisplayName("测试健康检查 - 健康")
    void testIsHealthy_Healthy() {
        when(aliyunOssStorageStrategy.isHealthy()).thenReturn(true);

        boolean result = storageService.isHealthy();

        assertTrue(result);
        verify(aliyunOssStorageStrategy, times(1)).isHealthy();
    }

    @Test
    @DisplayName("测试健康检查 - 不健康")
    void testIsHealthy_Unhealthy() {
        when(aliyunOssStorageStrategy.isHealthy()).thenReturn(false);
